...
def load_merscope(folder, library_id, scale_percent):
...
def filter_and_run_scanpy(adata, min_counts):
...
def save_merscope(adata, file):
    adata.obs = adata.obs.drop(['bounds'], axis=1)
    #del adata.uns['transcripts']
    adata.write(file)

def getFovCoordinates(fov, meta_cell):
...
def load_transcript(path, transformation_matrix, scale_percent):
